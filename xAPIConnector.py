...
class JsonSocket(object):
    ...
    encrypt = property(
        _get_encrypt, _set_encrypt, doc="read only property socket port"
    )
...
class APIStreamClient(JsonSocket):
    ...
    def subscribeTradeStatus(self):
        self.execute(
            dict(command="getTradeStatus", streamSessionId=self._ssId)
        )
    ...
    def unsubscribeTradeStatus(self):
        self.execute(
            dict(command="stopTradeStatus", streamSessionId=self._ssId)
        )
...
def loginCommand(userId, password, appName=""):
    return baseCommand(
        "login", dict(userId=userId, password=password, appName=appName)
    )
...
def main():
    ...
    if loginResponse["status"] == False:
        print(
            "Login failed. Error code: {0}".format(loginResponse["errorCode"])
        )
        return
